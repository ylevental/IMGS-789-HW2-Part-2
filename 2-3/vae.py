...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return z, self.decoder(z), mu, std
...
for epoch in range(1, 20):
    ...
    for batch_idx, (data, _) in enumerate(trainloader):
        ...
        data = data.cuda()
        bsize = data.shape[0]
        _, recon_batch, mu, std = model(data)
...
for i in range(1000):
  ...
  a = a.cuda()
  z, recon, mu, std = model(a[0])
  X = np.append(X, z[0].detach().cpu().numpy().reshape(1,-1), axis=0)
  c = np.append(c, t[0].detach().cpu().numpy())

X_embedded = TSNE(n_components=2).fit_transform(X)

print(np.shape(X_embedded))

scatter = plt.scatter(X_embedded[:,0], X_embedded[:,1], c = c, cmap='tab10')
plt.show()
