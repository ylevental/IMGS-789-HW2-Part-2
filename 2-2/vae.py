...
#Data from https://github.com/emalgorithm/gp-vs-cnn-uncertainty/tree/master/data
mat_data = sio.loadmat('mnist-with-awgn.mat')

testset_data = mat_data['test_x']
testset_targets = mat_data['test_y']

x_val = testset_data[:1000].reshape((-1,28,28))
y_val = testset_targets[:1000]
x_val = np.where(x_val > 128, 1, 0)
x_val = x_val.astype(np.float32)
...
for i in range(1000):
  ...
  #loss, err, kld = loss_function(recon, a[0], mu, std)
  #reconst_err.append(err.item())

  b = recon[0].reshape((28,28))
  err = mean_squared_error(a[0].detach().cpu().numpy(),b.detach().cpu().numpy())
  reconst_err.append(err)

print(len(reconst_err))     

# plotting labelled histogram
plt.hist(reconst_err)
plt.xlabel('Reconst. Error')
